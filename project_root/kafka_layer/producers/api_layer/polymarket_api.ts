...
export async function gammaFetch(endpoint: string, queryParams?: Record<string, string>) {
  try {
    if (!endpoint) {
      throw new Error("The endpoint argument is not defined");
    }
    ...
    throw new Error(`gammaFetch failed due to the following error: ${error}`);
  }
}
...
export async function fetchPoliticalEvents(limit: number = 30, skipCounter = 0) {
  try {
    ...
    throw new Error(` Failed to fetch the political data due to the following error: ${error}`);
  }
}
