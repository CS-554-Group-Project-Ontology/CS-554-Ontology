...
const AffordabilitySF = () => {
  ...
  useEffect(() => {
    ...
    // Add the SF neighborhoods layer once the map has loaded
    map.on('load', () => {
      ...
      map.on('mouseenter', 'sf-neighborhoods-fill', (event) => {
      ...
      map.on('mousemove', 'sf-neighborhoods-fill', (event) => {
        map.getCanvas().style.cursor = 'pointer';
        ...
        if (hoveredId !== null) {
          map.setFeatureState(
            { source: 'sf-neighborhoods', id: hoveredId },
            { hover: false },
          );
        }
      ...
      map.on('mouseleave', 'sf-neighborhoods-fill', () => {
  ...
  useEffect(() => {
    if (
      !isMapLoaded ||
      !mapRef.current ||
      !mapRef.current.getLayer('sf-neighborhoods-fill')
    ) {
      return;
    }

    mapRef.current.setPaintProperty('sf-neighborhoods-fill', 'fill-color', [
      'case',
      ['boolean', ['feature-state', 'hover'], false],
      'orange',
